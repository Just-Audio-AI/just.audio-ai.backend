...
class Products(Base):
    ...
    price_with_discount: Mapped[Optional[float]] = mapped_column(
        comment="Цена со скидкой"
    )
    discount_deadline: Mapped[Optional[datetime]] = mapped_column(
    ...
    is_subs: Mapped[bool] = mapped_column(
        default=False, comment="Подписка или разовая покупка"
    )
    ...
    gpt_request_limit_one_file: Mapped[Optional[int]] = mapped_column(
        comment="Лимит запросов в GPT по одному файлу"
    )
    vtt_file_ext_support: Mapped[bool] = mapped_column(
        comment="Поддерживается скачивание расшифровки в VTT формате"
    )
...
class Transactions(Base):
    ...
    external_transaction_id: Mapped[Optional[str]] = mapped_column(
        comment="Id транзакции в CloudPayments"
    )
...
class UserProducts(Base):
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column(
        comment="Дата окончания подписке"
    )
    is_subscription: Mapped[bool] = mapped_column(
        default=False, comment="Является ли записью подписки"
    )
    subscription_id: Mapped[Optional[str]] = mapped_column(
        comment="ID подписки в CloudPayments"
    )
