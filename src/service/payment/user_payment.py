...
class UserPaymentService:
    ...
    async def get_transaction_with_product(self, transaction_id: UUID) -> TransactionWithProduct:
        """Get a transaction with its product details"""
        try:
            transaction, product = await self.user_payment_repository.get_transaction_with_product(transaction_id)
            return TransactionWithProduct(transaction=transaction, product=product)
        except Exception:
            # If join fails, try to get just the transaction
            transactions = await self.user_payment_repository.get_user_transactions(user_id=None, transaction_id=transaction_id)
            if not transactions:
